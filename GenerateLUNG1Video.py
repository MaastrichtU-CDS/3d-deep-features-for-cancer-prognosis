...
root_source_path='A:/Dataset/NSCLC-Radiomics/NSCLC-Radiomics/'

video_store_path='A:/Dataset/Comparison Study of Radiomics and Deep Radiomics/Videos/'

patient_ID=os.listdir(root_source_path)

for i in range(400,401):#len(patient_ID)):
    ...
    second_patient_ID=os.listdir(patient_path)
    
    for t in range(len(second_patient_ID)):
        if 'CTLUNG' in second_patient_ID[t]:
            second_mask_path=patient_path+'/'+second_patient_ID[t]
        else:
            second_patient_path=patient_path+'/'+second_patient_ID[t]    
            image_series_path=second_patient_path+'/'+os.listdir(second_patient_path)[0]
    

    
    
    
    mask_ID=os.listdir(second_mask_path)
    
    for j in range(len(mask_ID)):

        if 'Segmentation' in mask_ID[j]:
            mask_path=second_mask_path+'/'+mask_ID[j]
            mask_name_list=os.listdir(mask_path)
            mask_name=mask_path+'/'+mask_name_list[0]
    #     else:
            
    #         image_slice_list=os.listdir(second_patient_path+'/'+image_ID[j])
            
    #         if len(image_slice_list)>1:
                
    #             image_series_path=second_patient_path+'/'+image_ID[j]
